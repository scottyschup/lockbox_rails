...
describe UpdateSupportRequest do
  ...
  it 'updates the support request' do
    ...
    result = UpdateSupportRequest.call(params: {id: support_request.id, support_request: {client_ref_id: '5678'}})
  ...
  it 'creates a note if certain support request fields were changed' do
    ...
    new_values.each do |field, value|
      expect{UpdateSupportRequest.call(params: {id: support_request.id, support_request: {field => value}})}
  ...
  it 'creates a note if the total amount was changed' do
    ...
    expect{UpdateSupportRequest.call(params: update_params)}
      .to change{Note.count}
      .by(1)
  ...
  it 'creates a single note if multiple fields changed' do
      expect{UpdateSupportRequest.call(params: {id: support_request.id, support_request: {client_ref_id: 'new client ref', name_or_alias: 'new name', urgency_flag: 'new urgency'}})}
        .to change{Note.count}
        .by(1)
